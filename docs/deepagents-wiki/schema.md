# Deep Agents Wiki Schema

> Governance document for the Deep Agents Architecture Wiki.
> Defines page types, naming, source references, and maintenance workflow.
...
- **Name:** Deep Agents Architecture Wiki
- **Purpose:** Human-readable and machine-readable knowledge base documenting the `deepagents/` monorepo: SDK, CLI, ACP bridge, eval stack, partner backends, and example agents.
- **Created:** 2026-04-06
...
|-------|----------|-------|------------|
| **Raw Sources** | `deepagents/` | Human-curated repository | Immutable by wiki operations |
...
Document a concrete subsystem or package boundary in the monorepo.
...
Important APIs, classes, middleware, or runtime concepts.
...
Document a pattern that spans packages or entities.
...
## Involved Entities
Which wiki entities implement or rely on this concept.

## Source Evidence
Key source files demonstrating the pattern.
...
Document how multiple entities compose into higher-level behavior.
...
High-level orientation material. These pages are freer-form but should link outward aggressively.

## Naming Conventions

- **File names:** lowercase-kebab-case with `.md`
- **Page titles:** Title Case
- **Source references:** backtick-wrapped paths relative to `deepagents/`
